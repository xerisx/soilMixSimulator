...
function fillInstantly(onComplete) {
  if (!currentCupDims) { if (onComplete) onComplete(); return; }
  ...
  function runChunk() {
    ...
    const end = Math.min(step + CHUNK, MAX_STEPS);
...
function cancelFill() { _fillCancelled = true; }
